# Guía de Implementación del Chat en Tiempo Real

## ✅ Cambios Realizados

### 1. Servidor WebSocket (`chat-server/`)
- **Ubicación**: Carpeta `chat-server` en la raíz del proyecto
- **Archivo principal**: `server.js`
- **Puerto**: 8080

### 2. Modificaciones en el Backend Django

#### `chat/models.py`
- ✅ Añadido método `tiene_mensajes()` para verificar si un chat tiene mensajes
- ✅ Añadido método `__str__()` para mejor representación en admin

#### `chat/serializers.py`
- ✅ Actualizado `MensajeSerializer` para usar `remitente` en lugar de `emisor`
- ✅ Actualizado `ChatSerializer` con:
  - Información de la mascota (nombre y foto)
  - Campo `tiene_mensajes`
  - Campo `fecha_creacion`
  - Remitente en el último mensaje

#### `chat/views.py`
- ✅ Modificado `get_queryset()` para:
  - **Adoptantes**: Ven todos sus chats (incluso sin mensajes)
  - **Protectoras**: Solo ven chats que tienen al menos un mensaje
- ✅ Añadido endpoint `obtener_o_crear` para crear/obtener chat de una mascota
- ✅ Corregido `enviar_mensaje` para usar `remitente` en lugar de `emisor`

#### `mascotas/views.py`
- ✅ Modificado `swipe_action` para:
  - Crear automáticamente un chat cuando se hace LIKE a una mascota
  - Retornar el `chat_id` creado en la respuesta

### 3. Flujo de Funcionamiento

```
1. Usuario hace "Me gusta" a una mascota
   ↓
2. Se crea automáticamente un chat entre adoptante y protectora
   ↓
3. El chat NO aparece en la bandeja de la protectora todavía
   ↓
4. El adoptante puede ver el chat y enviar mensajes
   ↓
5. Cuando el adoptante envía el PRIMER mensaje
   ↓
6. El chat aparece en la bandeja de la protectora
   ↓
7. Ambos pueden chatear en tiempo real vía WebSocket
```

## 📋 Pasos Pendientes para Completar

### Paso 1: Arreglar Migraciones
Hay un problema con las migraciones de mascotas. Necesitas:

```bash
cd PetConnect

# Opción A: Si NO tienes datos importantes, resetear las migraciones
# Elimina db.sqlite3 y todas las migraciones excepto __init__.py

# Opción B: Crear la migración faltante 0003 manualmente
# o renombrar 0004 a 0003 y ajustar las dependencias
```

### Paso 2: Aplicar Migraciones
```bash
cd PetConnect
& "C:/Users/Tarda/Documents/pythonIANúria(tarde)/proyecto final/PetConnect/PetConnect/.venv/Scripts/python.exe" manage.py makemigrations
& "C:/Users/Tarda/Documents/pythonIANúria(tarde)/proyecto final/PetConnect/PetConnect/.venv/Scripts/python.exe" manage.py migrate
```

### Paso 3: Instalar Dependencias del Servidor WebSocket
```bash
cd chat-server
npm install
```

### Paso 4: Ejecutar el Servidor WebSocket
```bash
cd chat-server
npm run dev
```
